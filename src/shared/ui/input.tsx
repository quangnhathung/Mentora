...
export const Input = React.forwardRef<NTextInput, NInputProps>((props, ref) => {
  const { label, icon, maxLengthRender, dark, disabled, editable, error, testID, ...inputProps } =
    props;
    ...
    <View className={styles.container()}>
      {label && (
        <View className={`flex-row justify-between`}>
          ...
          {icon === 'verified' && (
            <Text className={'bottom-1 self-end text-xs text-secondary dark:text-secondary'}>
...
export function ControlledInput<T extends FieldValues>(props: ControlledInputProps<T>) {
  const { name, calendar, icon, control, onChangeText, rules, ...inputProps } = props;
    ...
    return (
      <View className={styles.container()}>
        {inputProps.label && (
          <View className={`flex-row justify-between`}>
            <Text
              testID={inputProps.testID ? `${inputProps.testID}-label` : undefined}
