  ...
  (
    ...
    if (node.name === 'p') {
      ...
      return (
        <View
          key={index}
          className={twMerge(
            `flex-1 flex-row items-center justify-center ${pClass}`
          )}
        >
          {node.children.map((child: any, i: number) =>
            createRenderNode(oneLine)(
              child,
              i,
              node.children,
              node,
    ...
    if (node.name === 'b') {
      return (
        <Text
          key={index}
          className="items-center justify-center font-bevietnampro-bold text-white"
        >
          {node.children.map((child: any, i: number) =>
            createRenderNode(oneLine)(
              child,
