...
export const CheckboxIcon = ({ checked = false }: IconProps) => {
  const color = checked ? colors.primary[100] : colors.charcoal[400];
  return (
    <MotiView
      style={{
        height: SIZE,
...
export const RadioIcon = ({ hidden = false, checked = false }: IconProps) => {
  const color = checked ? colors.primary.DEFAULT : colors.border.white;
  return (
    <MotiView
      style={{ height: SIZE, width: SIZE, borderColor: color }}
      className={`items-center justify-center rounded-[22px] border-2 bg-transparent ${hidden && 'opacity-0'}`}
      from={{ borderColor: colors.border.white }}
      animate={{ borderColor: color }}
      transition={{ borderColor: { duration: 100, type: 'timing' } }}
    >
      <MotiView
        // luôn để màu, kiểm soát hiển thị bằng scale/opacity
        className="size-[12px] rounded-[12px] bg-primary"
        ...
        transition={{
          opacity: { duration: 150, type: 'timing' },
          scale: { duration: 300, type: 'timing' },
        }}
      />
    </MotiView>
  );
...
export const SwitchIcon = ({ checked = false }: IconProps) => {
  ...
  const backgroundColor = checked ? colors.primary[100] : colors.charcoal[400];
