...
export const getStatusChoice: <T>(
  ...
  if (!showAnswer) {
    return isCurrentRenderingChoice
      ? BaseChoiceStatus.SELECTED
      : BaseChoiceStatus.UNSELECTED;
  }
  // Hiển thị đáp án => SELECTED hoặc UNSELECTED hoặc WRONG hoặc CORRECT
  else {
    if (isCurrentRenderingChoice) {
      if (correctAnswerId === currentUserChoiceId)
        return BaseChoiceStatus.CORRECT;
